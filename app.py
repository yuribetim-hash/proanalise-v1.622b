...
<style>
    ...
    .painel-comando {{
        ...
        border: 1px solid {tema["cores"]["primaria_clara"]};
        border-radius: 12px;
        padding: 15px;
    ...
    .painel-comando .stButton > button {{
        width: 100%;
        margin: 5px 0;
    }}
...
if st.session_state.get("analise_ativa", False):
    ...
    if st.session_state.get("tempo_inicio"):
        tempo_decorrido = get_horario_brasilia() - st.session_state["tempo_inicio"]
...
if st.session_state.get("etapa") not in menus_disponiveis:
    st.session_state["etapa"] = menus_disponiveis[0] if menus_disponiveis else "1. Protocolo"
...
# ============================================
# FUNÇÃO DO PAINEL DE COMANDO
# ============================================
def render_painel_comando():
    """Renderiza o painel de comando flutuante na etapa de análise"""
    
    with st.container():
        st.markdown(f"""
        <div class='painel-comando'>
            <h4 style='margin-top: 0; text-align: center;'>🎮 Painel de Controle</h4>
            <hr style='margin: 8px 0;'>
        </div>
        """, unsafe_allow_html=True)
        
        # Botão Salvar manual
        if st.button("💾 Salvar manual", key="btn_salvar_manual_painel", use_container_width=True):
        ...
        nova_anotacao = st.text_area("📓 Anotações (não vão ao parecer)", value=anotacao_atual, height=100)
        ...
        if backups_disp:
            backup_selecionado = st.selectbox("🔄 Restaurar", list(backups_disp.keys()), 
                                               format_func=lambda x: backups_disp[x],
                                               key="backup_select_painel")
            if st.button("Restaurar este backup", key="btn_restaurar_painel", use_container_width=True):
...
# ============================================
def carregar_perguntas_pelo_tipo():
    tipo = st.session_state.get("tipo", "Loteamento")
    tipo_analise = st.session_state.get("tipo_analise", "Aceite urbanístico")
    return carregar_perguntas_por_tipo(tipo, tipo_analise)

perguntas = carregar_perguntas_pelo_tipo()
...
def gerar_docx(dados, respostas, observacoes, conclusao, analista, matricula, setor, n_analise, pendencias_manuais=None):
    ...
    context = {
        ...
        "conclusao": conclusao,
        "data": f"Data: {formatar_horario_brasilia()}",
...
def visualizar_parecer_html(dados, respostas, observacoes, conclusao, analista, n_analise, pendencias_manuais=None):
    ...
    <div style="font-family: Arial, sans-serif; padding: 20px; border: 1px solid #ccc; border-radius: 10px; background-color: white;">
        ...
        <p><strong>Data:</strong> {formatar_horario_brasilia()}</p>
        <hr>
...
if st.session_state["etapa"] == "1. Protocolo":
    ...
    with col1:
        ...
        if tipo != st.session_state["tipo"]:
            ...
            perguntas = carregar_perguntas_pelo_tipo()
    with col2:
        tipo_analise = st.selectbox("Tipo de Análise", ["Aceite urbanístico", "Alvará"],
                                     index=0 if st.session_state["tipo_analise"] == "Aceite urbanístico" else 1, key="tipo_analise_select")
    ...
    if st.session_state.get("analise_ativa"):
        ...
        if st.button("🚀 Iniciar Análise", use_container_width=True, type="primary"):
            if st.session_state["protocolo"] and st.session_state["interessado"]:
                ...
                st.session_state["analise_concluida"] = False
                st.session_state["tempo_inicio"] = get_horario_brasilia()
                st.session_state["etapa"] = "2. Analista"
                st.rerun()
            else:
...
# ============================================
    ...
    tem_pendentes = proximo_id is not None
    
    # Painel de comando flutuante (apenas se houver análise ativa)
    if tem_pendentes:
        render_painel_comando()
    ...
    if not st.session_state.get("analise_concluida", False):
        if st.button("✅ Concluir Análise", use_container_width=True, type="primary"):
            if preenchidas == total:
                ...
                st.session_state["tempo_fim"] = get_horario_brasilia()
                st.session_state["analise_ativa"] = False
